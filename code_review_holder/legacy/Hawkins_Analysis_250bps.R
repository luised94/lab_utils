...
path_3 = "C:/Users/liusm/Desktop/ARS_and_Gene_Analysis/YeastMine/Query Output/ORF_info_2020-04-06.csv"
read.csv(path_3, header = TRUE) %>% mutate_if(is.factor, as.character) -> ORF_df
...
Rhee_df %<>% mutate_at(Rhee_columns[c(2,3,6,8,18,19)], factor)
...
Hawkins_pos_minus = Hawkins_df$Position - 250
Hawkins_pos_plus = Hawkins_df$Position + 250
...
for (ars in 1:length(Hawkins_array)){
  ...
  is_upstream_plus = Genome_St < Hawkins_pos_minus[ars] & Genome_End > Hawkins_pos_minus[ars] & Genome_St < Hawkins_pos_plus[ars] & 
    Genome_End < Hawkins_pos_plus[ars] 
  is_upstream_minus =  Genome_St > Hawkins_pos_minus[ars] & Genome_End <Hawkins_pos_minus[ars] & Genome_St <Hawkins_pos_plus[ars] & 
    Genome_End < Hawkins_pos_plus[ars]
  ...
  #logic for determining whether gene is upstream accounting for directionality
  is_downstream_plus = Genome_St > Hawkins_pos_minus[ars] & Genome_End > Hawkins_pos_minus[ars] & Genome_St < Hawkins_pos_plus[ars] & 
    Genome_End > Hawkins_pos_plus[ars]
  is_downstream_minus = Genome_St > Hawkins_pos_minus[ars] & Genome_End > Hawkins_pos_minus[ars] & Genome_St > Hawkins_pos_plus[ars] &
    Genome_End < Hawkins_pos_plus[ars]
...
#Plotting levels of PIC for adjacent genes vs Efficiency and Competency for convergent genes 
p1 <- Hawkins_df %>% filter(ARS_TXN_type == "Convergent" & TBPHawkins != 0) %>% ggplot(aes(x = TBPHawkins, y = Efficiency)) + geom_point() +geom_smooth(method = 'lm')
p2 <- Hawkins_df %>% filter(ARS_TXN_type != "Convergent" & TBPHawkins != 0) %>% ggplot(aes(x = TBPHawkins, y = Efficiency)) + geom_point() +geom_smooth(method = 'lm')

ggarrange(p1, p2)

Hawkins_df %>% filter(TBPHawkins != 0) %>% ggplot(aes(x = TBPHawkins, y = Efficiency)) + geom_point() +geom_smooth(method = 'lm') + facet_wrap(ARS_TXN_type~., nrow =2 )

Hawkins_df %>% filter(ARS_TXN_type == "Convergent" & TBPHawkins != 0) %>% ggplot(aes(x = TBPHawkins, y = Competence, color = ARS_TXN_type)) + geom_point() +geom_smooth(method = 'lm')

Hawkins_df %>% filter(ARS_TXN_type == "Convergent" & TBPHawkins != 0) %>% ggplot(aes(x = IIAHawkins, y = Efficiency, color = ARS_TXN_type)) + geom_point() +geom_smooth(method = 'lm')
Hawkins_df %>% filter(ARS_TXN_type == "Convergent" & TBPHawkins != 0) %>% ggplot(aes(x = IIAHawkins, y = Competence, color = ARS_TXN_typlibre)) + geom_point() +geom_smooth(method = 'lm')

Hawkins_df %>% filter(ARS_TXN_type == "Convergent" & TBPHawkins != 0) %>% ggplot(aes(x = Pol.IIHawkins, y = Efficiency, color = ARS_TXN_type)) + geom_point() +geom_smooth(method = 'lm')
Hawkins_df %>% filter(ARS_TXN_type == "Convergent" & TBPHawkins != 0) %>% ggplot(aes(x = Pol.IIHawkins, y = Competence, color = ARS_TXN_type)) + geom_point() +geom_smooth(method = 'lm')

Hawkins_df %>% filter(ARS_TXN_type == "Convergent" & TBPHawkins != 0) %>% ggplot(aes(x = IIAHawkins, y = Efficiency, color = ARS_TXN_type)) + geom_point() +geom_smooth(method = 'lm')
Hawkins_df %>% filter(ARS_TXN_type == "Convergent" & TBPHawkins != 0) %>% ggplot(aes(x = IIAHawkins, y = Competence, color = ARS_TXN_type)) + geom_point() +geom_smooth(method = 'lm')

p1 <- Hawkins_df %>% filter((TXN_Direction_Upstream == 1 | TXN_Direction_Downstream == -1) & TBPHawkins != 0) %>% ggplot(aes(x = IIAHawkins, y = Efficiency)) + geom_point() +geom_smooth(method = 'lm')
p2 <- Hawkins_df %>% filter((TXN_Direction_Upstream == -1 | TXN_Direction_Downstream == 1) & TBPHawkins != 0) %>% ggplot(aes(x = IIAHawkins, y = Efficiency)) + geom_point() +geom_smooth(method = 'lm')
p3 <- Hawkins_df %>% filter(TBPHawkins != 0) %>% ggplot(aes(x = IIAHawkins, y = Efficiency)) + geom_point() +geom_smooth(method = 'lm')
ggarrange(p1, p2, p3)


p1 <- Hawkins_df %>% filter((TXN_Direction_Upstream == 1 | TXN_Direction_Downstream == -1) & TBPHawkins != 0) %>% ggplot(aes(x = IIAHawkins, y = Competence)) + geom_point() +geom_smooth(method = 'lm')
p2 <- Hawkins_df %>% filter((TXN_Direction_Upstream == -1 | TXN_Direction_Downstream == 1) & TBPHawkins != 0) %>% ggplot(aes(x = IIAHawkins, y = Competence)) + geom_point() +geom_smooth(method = 'lm')
p3 <- Hawkins_df %>% filter(TBPHawkins != 0) %>% ggplot(aes(x = IIAHawkins, y = Competence)) + geom_point() +geom_smooth(method = 'lm')
ggarrange(p1, p2, p3)


# Hawkins_df %>% group_by(ARS_TXN_type) %>% ggplot(aes(x = ARS_TXN_type, y = ..count..)) + geom_bar()
...
Hawkins_df %>% filter(ARS_TXN_type != "None_Flanking" & TBPHawkins != 0) %>% ggplot(aes(x = Pol.IIHawkins, color = ARS_TXN_type)) + geom_density(alpha = .4)+ facet_wrap(ARS_TXN_type~., nrow = 2)
Hawkins_df %>% filter(ARS_TXN_type != "None_Flanking" & TBPHawkins != 0) %>% ggplot(aes(x = IIAHawkins, color = ARS_TXN_type)) + geom_density(alpha = .4)+ facet_wrap(ARS_TXN_type~., nrow = 2)
Hawkins_df %>% filter(ARS_TXN_type != "None_Flanking" & TBPHawkins != 0) %>% ggplot(aes(x = TBPHawkins, color = ARS_TXN_type)) + geom_density(alpha = .4)+ facet_wrap(ARS_TXN_type~., nrow = 2)
...
for (i in 1:length(x_index)){
  ...
  name_for_file <- paste("250bps",df_columns[x_index[i]], "density", "by","ARS_TXN_type", "filter", "None_FlankingAndNot0","col", "ARS_TXN_type", ".png", sep = "_")
