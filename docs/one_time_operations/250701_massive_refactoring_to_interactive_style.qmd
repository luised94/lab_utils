...
#### Session 3
Realized I should rename the configuration files as well.
Reuse the code above but make some modifications. Also create parameters.
```bash
OLD_PATTERN="configuration_bmc"
NEW_PATTERN="configuration_experiment_bmc"
SEARCH_DIR="$HOME/data/"
mapfile -t files_to_rename < <(find "$SEARCH_DIR" -type f -name "*${OLD_PATTERN}*")
for file in ${files_to_rename[@]}; do
  echo "Processing $file"
...
```
Renamed the other configuration files using standard move command.
There are probably some unintended effects of the modifications. I think the only way to figure out at this point is to run the scripts. I can just switch to the other branch if the errors are too complex.

### 2025-07-18
#### Session 1
I am going to perform the replacement of the banner styles.
```vim
"vimgrep /^#\+\n# [^:]*\n^#\{2,}/j core_scripts/**/*.R 
" Find the banners and populate the quickfixlist.
vimgrep /^#\+\n^# [^:]*\n^#\+\n/j core_scripts/**/*.R 
cn " Verify the matches
cp " Go back to the first.
" Use a macro
qq
Ctrl+Q " Visual block mode
3j
" s/^#\zs#\+/\=repeat('-', len(submatch(0)))/ should also work.
'<,'>s/^\(#\+$\)/\='#' . repeat('-', len(submatch(1)) - 1)/
cn
q
@q " Just do one at a time. Maybe using % with gc would have made it more efficient and interactive
```
The reason why I didnt use cdo or recursive macro is that for some reason, one of the matches did not go to the first hash line but to the middle. Has to be an error in the regex but did not try to figure out and went with the just get it done now approach.

#### Session 2
Added some missing headers.
Ready to work on the replacement of the arguments with the interactive code blocks. I am trying to be "smart" and efficient about the method.
My first attempt:
```vim
" Find the assignment expressions.
/\w\+\s<-
" Grab the variable word and place into a new file using macro
vnew variables.txt
qqyiw<Ctrl+W>hpo<Esc><Ctrl+W>lnq
@q " However many times you want.
```

Got distracted trying to figure out how to how to execute the replacements. I was trying to figure out how to replace content in different ways.
```vim
" Edit | 2025-07-18: This did not work the second time I tried it. See session 3 for this date.
" All variations yank something into a reusable register
norm "ayy
" All require Visual Block mode
<Ctrl+Q> " or V.
" Select lines and go till the end.
" Replace each selected lines with content of register a
'<,'> norm "ap
'<,'> norm dd"aP
" Replaces but has space in between because of default put
'<,'>s/.*/\=@a/
" Replace all of the lines with the content of register a.
norm "ap
```
I can use this for replacing the contents now.
First, clean up the sections to replace of lines that are only newlines.
```vim
vimgrep /Handle script arguments/j core_scripts/**/*.R
cn
qq
/Handle script arguments
norm n
<Ctrl+Q>
norm /print_debug_info
'<,'>s/^\n//
cn
q
@q " However many times you need or based on the length of the quickfixlist.
```

#### Session 3
Will replace the argument handling with the interactive block. There is only one flow cytometry script. To ensure that I paste the compatible code, I handled this one by itself.
The replacement was causing the pasting to mess up the structure of the pasted code. Some of the lines would get joined together. Furthermore, the replacement would also join the lines instead of replacing. Just pasted and then deleted. Not sure what the issue was.

Now I had to make sure the variables were corrected. One issue was that when I made the setup script I had not decided on capital letters for presumably constant variables.
```vim
* " While on top of the variable to adjust
norm %s//\U&/g
```
Also wanted to see if I could adjust my indentation from 4 to 2 per level.
```vim
norm %s/\s\{4}/  /g
" or on a selection
'<,'>s/\s\{4}/  /g
...
### 2025-07-19
#### Session 1
Tried to replace the handle script argument sessions in the other one. Messed up the macro by not pasting the register. Furthermore, still had the issue were the contents were pasting in place.

```vim
norm '<,'>"ay
norm qq/Handle script arguments<CR>2-:pu<CR>n-<C-Q>}$"_d<C-Q>}{:d<CR>:cn<CR>q
" Pattern for deleting empty lines
<Ctrl+Q>}{d
```

Random thought: Macro for modifying variables (untested)
```vim
norm /^\w\+\s<-
qq*%s//<insert_mod_here>/gnq
```

#### Session 2
Useful command to reopen modified files in commit relative to head. Replace one with whatever index you want.
```bash
nvim $(git diff --name-only HEAD~1)
```

You can use the * trick for reusing variables in expression to find an entire expression to refactor. Couple with vimgrep and macro.
```vim
" Highlight the expression by visually selecting the entire line.
<Ctrl+Q>$*
" Record macro
norm qq<expression_to_refactor_here>cnq
" End with @q for recursive macro. run let @q = '' to ensure it is empty.
wa
```

### 2025-07-28
#### Session 1
Forgot I had to adjust the configuration for the experiments. Still considering if I should consolidate entirely or just denest experiment configuration specific and consolidate the sample things.
Started running the new scripts and encountering errors. First, I accidentally altered the arguments passed to handle_configuration_checkpoint function by making upper case when the arguments are case sensitive.
```nvim
vimgrep /handle_configuration_checkpoint(\n/ core_scripts/**/*.R
norm qq<CR>guiw<CR>guiw:cnq
```

### 2025-08-21
#### Session 1
Trying to create the info to submit new form and dealing with expected bugs.
Need to simplify a lot and refactor the configuration files as well.

#### Session 2
Denest some of the logic from functions and the configurations.

List of variable mentions to fix:
- RUNTIME_CONFIG$<value> with whatever is denested or just renamed
- "~/lab_utils/core_scripts/" with core_scripts_path
- TIME_CONFIG$<value>
- GENOMIC_TRACK_CONFIG$<value>
