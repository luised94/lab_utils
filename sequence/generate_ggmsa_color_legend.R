...
dummy_seq <- Biostrings::AAStringSet("GAVLIPFWYSTNQCMDEKRH")
...
# Layer 2 contains text labels
text_data <- build$data[[2]]

cat("  Tile layer columns:", paste(colnames(tile_data), collapse = ", "), "\n")
cat("  Text layer columns:", paste(colnames(text_data), collapse = ", "), "\n")

# Extract AA-to-color mapping from text layer (has 'label' column with AA letters)
# Match by position (x coordinate) to tile fill colors
color_map <- merge(
    x = text_data[, c("x", "label")],
    y = tile_data[, c("x", "fill")],
    by = "x"
)

# Deduplicate to get unique AA -> color mapping
color_map <- unique(color_map[, c("label", "fill")])
colnames(color_map) <- c("aa", "color")
