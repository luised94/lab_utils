...
stopifnot(
  "Only one xit file expected. Double check directory." = length(XIT_FILEPATH) == 1
)
...
ordered_metadata_index <- do.call(
  order,
  metadata[EXPERIMENT_CONFIG$COLUMN_ORDER]
)
metadata <- metadata[ordered_metadata_index, ]
...
for (filename in filenames) {
  output_file_path <- file.path(SERIES_DIRECTORY, paste0(EXPERIMENT_ID, "_", filename))
