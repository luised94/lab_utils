...
#
# DEPENDENCIES: ~/lab_utils/core_scripts/new_name.R
#
# OUTPUTS:
...
# DEPENDENCIES: R base packages only, functions_for_new_name_validation.R for validation functions
# COMMON ISSUES:
...
EXPERIMENT_CONFIG <- list(
    ...
    CATEGORIES = list(
        arrest_point = c( "none", "alpha", "noco"),
    ...
    INVALID_COMBINATIONS = list(
        input_restrictions = quote(
            antibody == "Input" &
            !( arrest_point == "none" & timepoints == "0" )
        ),
        zero_restrictions = quote(
            timepoints == "0" &
            !( arrest_point == "none" )
        )
    ),
    ...
    SAMPLE_CLASSIFICATIONS = list(
        is_positive = quote(
            (antibody == "HM1108" & rescue_allele == "WT")

        ),
        is_negative = quote(
            (antibody == "UM174" & arrest_point == "noco")
        ),
        is_input = quote(
            antibody == "Input"
        )
...
show_particular_metadata <- FALSE
...
GENOME_TRACK_CONFIG <- list(
    ...
    file_sample_id = "consolidated_([0-9]{4,5,6})_sequence\\.fastq",
...
source("~/lab_utils/core_scripts/functions_for_new_name_validation.R")
