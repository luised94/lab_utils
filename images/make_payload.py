...
img = tifffile.imread(
    "/mnt/user-data/uploads/20260818_rotated_LM-0008_s0012_SDSPAGE_run-old-MCM-aliquots_Fl-Green.tif"
)

N = 1000
roi_y = int(prof.roi_y.iloc[0])
roi_h = int(prof.roi_h.iloc[0])
mm_per_px = float(
    (prof.groupby("lane_index").migration_position_millimetres.last().iloc[0]) / (N - 1)
)
...
for li, sub in band.groupby("lane_index"):
    r = sub.iloc[0]
    ident[str(int(li))] = {
        "well": int(r.well_number),
        "label": r.sample_label,
        "role": r.analysis_role,
    }
