...
for FILE in ${FILES_TO_CONCATENATE[@]}; do 
  ...
  URL_TO_DOWNLOAD=${BASE_URL}${ACCESSION[FILE_INDEX]:0:6}/${ACCESSION[FILE_INDEX]}/${ACCESSION[FILE_INDEX]}.fasta.gz 
  printf "%s | %d | %s\n" "$FILE_INDEX" "$OUTPUT_FILE" "$URL_TO_DOWNLOAD"
  curl ${BASE_URL}${ACCESSION[FILE_INDEX]:0:6}/${ACCESSION[FILE_INDEX]}/
