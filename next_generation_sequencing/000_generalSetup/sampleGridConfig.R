...
main <- function(experiment_in_config_file) {
    ...
    # Define the columns that determine the control columns. Typically any category related to genotype.
    #@update
...
define_categories <- function() {
    ...
    all_categories_list <- list(
        ...
        mcm_tag = c("none", "7"),
        timepoint_after_release = c("0", "1", "2"),
        antibody = c("Input", "ProtG", "HM1108", "74", "11HA")
    )
...
filter_samples <- function(combinations_grid){
    ...
    return(combinations_grid[is_input | is_protg | is_1108 | is_174 | is_11HA , ])
}
...
generate_filtered_samples <- function(categories_list) {
    ...
    #@update
    reorder_index <- with(filtered_samples,
        order(antibody, strain_source, rescue_allele, mcm_tag, timepoint_after_release)
    )
...
add_comparisons <- function(ordered_samples_table) {
    ...
    comparisons <- list(
        ...
        #Effect of time course of UM174: Timecoure for wt strains for 174
        comp_timecourse174 = with(df,
                strain_source == "lemr" & antibody == "74"
        ),

        #Effect of time course on 11HA: Timecourse for no rescue tag oa strain for 11HA
        comp_timecourse11HA_notag = with(df,
                strain_source == "oa" & antibody == "11HA" & mcm_tag == "none"
        ),

        #Effect of time course on 11HA: Timecourse for 7 mcm tag for 11HA
        comp_timecourse11HA_7tag = with(df,
                strain_source == "oa" & antibody == "11HA" & mcm_tag == "7"
        ),

        #Effect of time course on 1108: Input, G1 and M phase for 174
        comp_timecourse1108 = with(df,
                strain_source == "lemr" & antibody == "HM1108"
        )
