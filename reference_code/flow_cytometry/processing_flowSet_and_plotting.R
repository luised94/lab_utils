...
=======
  ...
  cat(
    "Sample", sampleNames(filtered_set_iqr)[i],
    "retained", sum(keep), "/", nrow(frame_data), "events\n"
  )
}
...
=======
  geom_point(
    data = timepoint_medians, inherit.aes = FALSE,
    aes(
      x = median_FL1A,
      y = timepoints * 500, # Using the same offset logic as earlier
      fill = factor(timepoints),
...
=======
  labs(
    title = "Vertical Offset FL1-A Distributions",
    subtitle = "Median values indicated by outlined circles"
  ) +
>>>>>>> pipeline_completion
  theme_minimal()
...
for (i in seq_along(filtered_set_density)) {
  ...
  cat(
    "Sample", sampleNames(filtered_set_density)[i],
    "retained", sum(keep), "/", nrow(frame_data), "events\n"
  )
>>>>>>> pipeline_completion
}
