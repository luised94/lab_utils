...
for(tp in levels(data$Timepoint)) {
  for(smpl in unique(data$Sample)) {
    ...
    plot(h, col = hcl.colors(length(h$breaks), "TealGrn"),
         main = "", xlab = "", ylab = "",
    ...
    text(x = max(global_breaks) * 0.9, y = max_count * 0.9,
...
mtext(unique(data$Sample), side = 3, outer = TRUE, 
      at = seq(1/(2*samples), 1, length.out = samples),
      line = -2, font = 2)
mtext(levels(data$Timepoint), side = 4, outer = TRUE,
