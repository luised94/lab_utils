###############################################################################
# Plot bin size and smooth Length combinations for bigwig files
################################################################################
# PURPOSE: Understand the effect of bin size and smooth length parameters on genome track visuals
# Conclusion:
#   = 
# USAGE: source("reference_code/pipeline_completion/plot_bin_size_and_smooth_length_bw_combinations.R")
# DEPENDENCIES: GenomicRanges, rtracklayer
# OUTPUT: pdf plots with comparisons bin sizes by smooth length
# AUTHOR: LEMR
# DATE: 2025-05-27
# DATE_V1_COMPLETE: 
################################################################################
...
# Breakpoint
stop("[BREAKPOINT] Figure out how to subset bigwig files to the bin size and smoothLength subset...")
is_bigwig_file_to_process <- grepl(
  "_raw_[0-9]{1,4}_[0-9]{1,4}\\.bw$",
  BIGWIG_FILES,
  ignore.case = FALSE
  )
BIGWIG_FILES_TO_PROCESS <- BIGWIG_FILES[is_bigwig_file_to_process]
#-------------------------------------------------------------------------------
