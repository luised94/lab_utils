...
svglite(plot_output, width = 6, height = 4)
ggplot(combined_data, aes(x = `FITC.Width`, y = after_stat(density), fill = sample_id)) +
	geom_density(alpha = .2, position = position_nudge(y = combined_data$offset)) +
	xlim(0, median(combined_data$FITC.Width) + 1500) +
	labs(title = "Fluorescence Intensity Distribution", x = "Fluorescence Intensity", y = "Density")
dev.off()

#Use a facet grid to plot the samples on different plots
plot_output <- paste0(output_directory, get_current_datetime_string(), "_plot_count.svg")
svglite(plot_output, width = 6, height = 4)
ggplot(combined_data, aes(x = `FITC.Width`)) +
        geom_density(alpha = .2, fill = "#FF6666") +
...
#JUNK
#scale_fill_gradient(colors = rainbow(length(unique(combined_data$sample_id)))) + 
#plot_output <- paste0(output_directory, "/", "plot_hist.svg")
#svglite(plot_output, width = 6, height = 4)
#hist(exprs(fcs_data[,'FITC-Width']),
#	  main = "Histogram of FITC-Width",
